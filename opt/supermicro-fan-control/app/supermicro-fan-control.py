...
def set_fan_speed(speed):
    ...
    # 根据测试结果调整计算公式，X10SRH-CLN4F 可能有特殊的转速曲线
    # 我们发现较低的速度值（如 10%）可能会导致更高的实际转速
    # 调整公式以更好地匹配观察到的行为
    ...
    for fan_zone in CONFIG["ipmi"]["fan_zones"]:
        ...
        # 根据 X10SRH-CLN4F 测试结果优化转速计算公式
        # 测试发现：
        # - 0x0E-0x12 (14-18) → 2400 RPM (最佳范围)
        # - 低于0x0E → 转速反而升高
        # - 高于0x12 → 转速继续增加
        
        # 使用分段优化：
        # 0-30%: 直接映射到 0x0E-0x10 (最佳低转速范围)
        # 30-60%: 映射到 0x10-0x1E
        # 60-100%: 映射到 0x1E-0x64
        
        if speed <= 30:
            # 低转速：直接使用最佳范围 0x0E-0x10
            fan_zone_speed_dec = 0x0E + int((0x10 - 0x0E) * speed / 30)
        elif speed <= 60:
            # 中转速：0x10-0x1E
            fan_zone_speed_dec = 0x10 + int((0x1E - 0x10) * (speed - 30) / 30)
        else:
            # 高转速：0x1E-0x64 (100%)
            fan_zone_speed_dec = 0x1E + int((0x64 - 0x1E) * (speed - 60) / 40)

        # 确保结果在有效范围内
        fan_zone_speed_dec = max(fan_zone_min_speed_dec, min(fan_zone_max_speed_dec, int(fan_zone_speed_dec)))

        # Calculate HEX Speed
        fan_zone_speed_hex = format(fan_zone_speed_dec , "02x")

        # Echo
        log(f"Fan Controller: Setting Fan Zone {fan_zone_id} ({fan_zone_description}) to {speed}% (Calculated: {fan_zone_speed_dec}%, Hex: 0x{fan_zone_speed_hex})" , "INFO")
...
def loop():
    while True:
        ...
        # Get other component temperatures (memory, PCH, etc.)
        other_temps = []
        
        # Get all temperature sensors from IPMI
        cmd = ["ipmitool", "sdr", "type", "temperature"]
        temp_output_obj = Command(command = cmd , return_result = True , check_return_code = True)
        time.sleep(2)
        temp_output = temp_output_obj.getOutput(decode=True)
        
        # Extract temperatures for DIMM, PCH, VRM, etc.
        for line in temp_output.split("\n"):
            if any(keyword in line for keyword in ["DIMM", "PCH", "VRM", "System", "Peripheral"]):
                match = re.search(r"(\d+\.\d+)\s*degrees", line)
                if match:
                    temp = float(match.group(1))
                    other_temps.append(temp)
                    log(f"Component Temperature: {line.strip()}" , level="DEBUG")
        
        max_other_temp = max(other_temps) if other_temps else None
        ...
        new_fan_speed_other = current_fan_speed
        ...
        # Protect other components (memory, PCH, etc.)
        if max_other_temp is not None:
            # For other components, we use CPU thresholds for now
            if max_other_temp >= CONFIG["cpu"]["warning_temp"]:
                log(f"Other Component OverTemperature Protection: Temperature = {max_other_temp}°C is higher than the Warning Setting = {CONFIG['cpu']['warning_temp']}°C" , level="WARNING")
                log(f"Other Component OverTemperature Protection: Sounding BEEP on the Speaker" , level="WARNING")
                os.system(f"beep -f {CONFIG['beep']['frequency']} -l {CONFIG['beep']['duration']} -r {CONFIG['beep']['repetitions']} -D {CONFIG['beep']['delay']}")
            
            if max_other_temp >= CONFIG["cpu"]["shutdown_temp"]:
                log(f"Other Component OverTemperature Protection: Temperature = {max_other_temp}°C is higher than the Shutdown Setting = {CONFIG['cpu']['shutdown_temp']}°C" , level="CRITICAL")
                log(f"Other Component OverTemperature Protection: Shutting Down System Now" , level="CRITICAL")
                time.sleep(2)
                os.system(f"shutdown -h now")
                
            # Regulate Fan Speed based on other components
            new_fan_speed_other = run_temperature_controller(label = "Other" , id = "other_components" , current_temp = max_other_temp , current_fan_speed = new_fan_speed_other)




        # Get worst Case - include other components
        new_fan_speed = max([new_fan_speed_cpu , new_fan_speed_drive , new_fan_speed_hdd , new_fan_speed_ssd , new_fan_speed_nvme , new_fan_speed_other])
